...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get membership type as a string (for storing in bills)
    ...
    $membershipType = $membershipTypeRow['type'];

    // Insert bill record
    $insertBillQuery = "INSERT INTO bills (member_id, membership_type, amount, renew_duration, total_amount, renew_date, expiry_date) 
                        VALUES ($memberId, '$membershipType', $totalAmount, $renewDuration, $totalAmount, '$renewDate', '$expiryDate')";
    $insertBillResult = $conn->query($insertBillQuery);

    if ($updateMemberResult && $insertRenewResult && $insertBillResult) {
        // Fetch the last inserted bill ID
        $billId = $conn->insert_id;

        // Redirect to view the bill page
